...
import { ArrowLeft, Plus, Save, Settings, Play } from 'lucide-react';
  ...
  return (
    <div className="absolute top-0 left-0 right-0 z-10 p-4 flex justify-between items-start pointer-events-none">
      <div className={`backdrop-blur shadow-xl border rounded-2xl p-2 pointer-events-auto flex items-center space-x-3 pr-4 transition-all ${isDirectorMode ? 'bg-slate-900/80 border-white/10 ring-1 ring-white/5' : 'bg-white/90 border-slate-200'}`}>
        ...
        {/* Back Button */}
        <button onClick={onBack} className={`p-2 rounded-xl transition-colors ${isDirectorMode ? 'hover:bg-slate-800 text-slate-400' : 'hover:bg-slate-100 text-slate-500'}`}>
        ...
        <Toggle 
          ...
          label={isLinkMode ? (isDirectorMode ? "LINK ACTIVATED" : "连线开启") : "连接模式"} 
        />
